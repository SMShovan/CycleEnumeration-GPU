...
class ReadTarjanTimeWindowSearch {
 public:
  ReadTarjanTimeWindowSearch(const GraphView& graph,
  ...
  CycleHistogram run() {
    for (VertexId root = 0; root < graph_.vertex_count(); ++root) {
      ...
      for (std::size_t offset = begin; offset < end; ++offset) {
        const AdjacencyEntry& start_edge = graph_.outgoing_edges()[offset];
        for (std::size_t timestamp_offset = start_edge.timestamp_begin;
          ...
          std::vector<unsigned char> blocked(graph_.vertex_count(), 0);
          Path path;
          if (find_path(start_edge.vertex, blocked, path)) {
            follow_path(root, blocked, std::move(path));
          }
        }
      }
    }
 ...
 private:
  ...
  bool find_path_dfs(const VertexId vertex,
    ...
    for (std::size_t offset = begin; offset < end; ++offset) {
      ...
      if (visited[next] != 0 || blocked[next] != 0) {
        continue;
      }
  ...
  void cycles_read_tarjan(VertexId edge_vertex,
    ...
    for (std::size_t offset = begin; offset < end; ++offset) {
      const AdjacencyEntry& edge = graph_.outgoing_edges()[offset];
      const VertexId next = edge.vertex;
      if (blocked_state[next] != 0 || !edge_has_timestamp(edge, edge_vertex)) {
        continue;
      }
  ...
  void follow_path(const VertexId edge_vertex,
    ...
    while (path.back() != root_ && !alternate_path.has_value()) {
      ...
      for (std::size_t offset = begin; offset < end; ++offset) {
        const AdjacencyEntry& edge = graph_.outgoing_edges()[offset];
        const VertexId candidate = edge.vertex;
        if (candidate == path.back() || blocked[candidate] != 0 ||
            !edge_has_timestamp(edge, previous)) {
          continue;
        }
...
CycleHistogram count_time_window_cycles_read_tarjan(
    const GraphView& graph,
    const Timestamp window_width,
    const std::optional<std::size_t> max_cycle_length) {
  return ReadTarjanTimeWindowSearch(graph, window_width, max_cycle_length).run();
}
